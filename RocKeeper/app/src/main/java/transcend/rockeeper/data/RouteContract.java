...
public class RouteContract extends Contract implements BaseColumns {
	...
	// A factory function for creating a Route object
	public Route build(String difficulty, int attempts, long loc_id, int color,
			String name, int completed, int points) {
	...
	public class Route extends Unit {
		public Route(Long _id, String difficulty, int attempts, long loc_id,
				int color, String name, int completed, int points) {
			if (_id != null)
				put(_ID, _id);
	...
	// Factory function that builds a Route from a cursor pointing to a result
	// from a query
	public Route build(Cursor c) {
		return this.new Route(c.getLong(c.getColumnIndex(_ID)), c.getString(c
				.getColumnIndex(DIFFICULTY)), c.getInt(c
				.getColumnIndex(NUM_ATTEMPTS)), c.getLong(c
				.getColumnIndex(LOCATION)), c.getInt(c.getColumnIndex(COLOR)),
				c.getString(c.getColumnIndex(NAME)), c.getInt(c
						.getColumnIndex(COMPLETED)), c.getInt(c
